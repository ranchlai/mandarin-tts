...
class FS2TransformerDecoder(nn.Module):
    ...
    The input sequence is a list of tensors, which may contain text-embedding, speaker-embedings.
    ...
    def __init__(
        ...
        pos_table = get_sinusoid_encoding_table(n_position,
                                                input_dim).unsqueeze(0)

        self.position_enc = nn.Parameter(pos_table, requires_grad=False)
        layers = []
        for _ in range(n_layers):
            layer = FFTBlock(hidden_dim,
                             d_inner,
                             n_heads,
                             d_k,
                             d_v,
    ...
    def forward(self, input: Tensor, mask):
        ...
        if input.shape[1] != seq_len:
            raise ValueError(
                'The input sequences must have the same dimension')
        ...
        pos_embed = self.position_enc[:, :seq_len, :].expand(
            batch_size, -1, -1)

        output = input + pos_embed
        for layer in self.layers:
            output, dec_slf_attn = layer(output,
                                         mask=mask,
                                         attn_mask=attn_mask)
