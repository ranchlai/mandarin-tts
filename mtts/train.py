...
class AverageMeter:
    ...
    def update(self, mel_loss, posnet_loss, d_loss, total_loss):
        self.mel_loss_v = (
            (self.mel_loss_v * self._i) + mel_loss.item()) / (self._i + 1)
        self.posnet_loss_v = ((self.posnet_loss_v * self._i) +
                              posnet_loss.item()) / (self._i + 1)
        self.d_loss_v = (
            (self.d_loss_v * self._i) + d_loss.item()) / (self._i + 1)
        self.total_loss_v = (
            (self.total_loss_v * self._i) + total_loss.item()) / (self._i + 1)
...
if __name__ == '__main__':
    ...
    lrs = np.linspace(0, optim_conf['params']['lr'],
                      optim_conf['n_warm_up_step'])
        ...
        for i, data in enumerate(dataloader):
            ...
            for k in range(config['training']['batch_split']):
               # import pdb;pdb.set_trace()
                ...
                mel_loss, mel_postnet_loss, d_loss = loss_fn(
                    d_pred, duration, mel_pred, mel_postnet, mel_truth, ~src_mask,
                    ~mel_mask)

                total_loss = mel_postnet_loss + d_loss + mel_loss
                ml, pl, dl, tl = avg.update(mel_loss, mel_postnet_loss, d_loss,
                                            total_loss)
